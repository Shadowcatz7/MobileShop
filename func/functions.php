...
// request method post
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    if (isset($_GET['id'], $_POST['account-insert'])) {
        $id = $_GET['id'];
        
        if (isset($_POST['username-' . $id], $_POST['password-' . $id], $_POST['email-' . $id], $_POST['privilege-' . $id])) {
            $username = $_POST['username-' . $id];
            $password = $_POST['password-' . $id];
            $email = $_POST['email-' . $id];
            $privilege = $_POST['privilege-' . $id];

            $acc->insertAcc($id, $username, $password, $email, $privilege);
        }
    } else {
