...
//  echo "<script>bootbox.alert('This is the default alert!');
//                     </script>";
                return false;
            }
        }
...
// Kiểm tra xem thêm vào bảng user có thành công không
if ($resultUser) {
    // Lấy giá trị id vừa được thêm vào bảng user
    $userId = $this->db->con->insert_id;

    // Thêm dữ liệu vào bảng account và sử dụng giá trị id từ bảng user
    $sqlAccount = "INSERT INTO account(username, password, email, user_id) VALUES ('{$username}','{$password}','{$email}', '{$userId}')";
    $resultAcc = $this->db->con->query($sqlAccount);

    // Kiểm tra xem thêm vào bảng account có thành công không
    if ($resultAcc) {
        // Thêm thành công cả hai bảng
        echo "Thêm thành công!";
    } else {
        // Xóa hàng đã thêm vào bảng user nếu thêm vào bảng account không thành công
        $this->db->con->query("DELETE FROM user WHERE id = '{$userId}'");
        echo "Thêm vào bảng account không thành công!";
    }
} else {
    echo "Thêm vào bảng user không thành công!";
}
